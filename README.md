...
港區的 AI 數位員工：以自然語言下指令，透過 7 個工具讀取／導航／操作六大功能模組的資料層，
把整個生態系串起來——跨模組整合問答（如「整合今日港區營運摘要」「紅海事件對碳成本的影響？」）、
系統自我檢測（確定性 probe + 維運 runbook，異常時給修復步驟）、以及需人工確認的寫入操作（碳權
掛單、修改設定）。進頁自動跑一次靜默巡檢，右側工作區以 6+1 燈號牆逐卡點燈；任務執行採
plan-then-act（計畫時間軸打勾）+ 常駐旁白字幕 + 工作區跟隨（工具結果卡堆疊、完成後留下模組
足跡 chips）；答案中的模組引用以彩色 citation chip 呈現，hover 可看該模組本次工具摘要、點擊
直接跳轉該頁；碳權掛單／改設定這類寫入動作一律先出確認卡，操作員按下才會真的送出。以
human-in-the-loop 為設計語言——AI 不是黑箱操作者，而是每一步都看得見、可中斷、可確認的協作者。

![數位員工](docs/screens/agent.png)

> 本頁為雙態 provider：`.env` 設定 `VITE_GEMINI_API_KEY` 後走真實 Gemini（`gemini-2.5-flash`）
> tool-calling，標題列 chip 顯示「GEMINI LIVE」；未設定 key 則自動退回四條劇本 mock（chip 顯示
> 「劇本 MOCK」）——劇本內的資料查詢工具仍會真的呼叫各模組 provider（資料是活的，只有回答文字
> 預錄），碳權掛單／系統健檢等工具在 mock 態下同樣可完整互動。Gemini key 僅限本機 demo，
> **勿提交版控、勿部署公開網址**（瀏覽器直接呼叫 API，key 會出現在 network 請求中）。
> 本輪驗收已用測試用（非真實）key 驗證「切換到 live 模式＋Gemini 回錯時的優雅降級」路徑（標題列
> chip 正確切換、錯誤事件列出現、頁面不崩潰、拔除 key 後正確退回 mock）；真實 key 的完整成功案例
> （真答案＋真工具呼叫＋掛單真打碳權 API）留待使用者以自己的 Gemini key 驗證。

### 系統設定（`settings`）

全站前後端設定頁（左側 rail 底部齒輪進入，快捷鍵 `8`）：schema 驅動的設定框架，左欄七個分區
...
| `VITE_GEMINI_API_KEY` | 數位員工（agent）頁 Gemini API key；未設定則退回劇本 mock，**選填** | （無，需自行填入） |
...
`VITE_GEMINI_API_KEY` 未設定時數位員工頁自動退回劇本 mock，不影響 demo；**此 key 僅限本機
使用，絕不可提交版控或部署到公開網址**（瀏覽器直接呼叫 Gemini API，key 會出現在 network
請求中）。`.env` 已列入 `.gitignore`，變數不會進版控。
...
tool-calling，於 `.env` 填入 `VITE_GEMINI_API_KEY`（[Google AI Studio](https://aistudio.google.com/)
免費取得）後重啟 `npm run dev`，標題列 chip 會切成「GEMINI LIVE」。**此 key 僅限本機 demo 使用，
絕不可提交版控、絕不可部署到公開網址**（瀏覽器直接呼叫 API，key 會出現在 network 請求中；競賽
若需公開部署，屆時應改走後端代理簽名，不應沿用本機直呼架構）。碳權掛單工具在 `VITE_CARBON_API`
未啟動時會優雅退回示範回覆（訊息帶「示範」），不影響 live 對話本身。
