...
- spec：`docs/superpowers/specs/2026-07-12-ppt-presentation-demo-design.md`；plan：`docs/superpowers/plans/2026-07-12-ppt-presentation-demo.md`（10 tasks）；ledger `.superpowers/sdd/progress.md`（scratch，未進版控）。
- **下一步**：使用者人眼看一輪 9 支影片動態品質 → 決定 finishing（合併方式，比照前例可能本地 ff 合併回 main 或 push）。錄影管線可再生成（`npm run demo:record -- <名>`），任一頁後端上線後單支重錄（索引見簡報腳本 §13）。
